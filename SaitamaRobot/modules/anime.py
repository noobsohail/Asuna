...
def site_search(update: Update, context: CallbackContext, site: str):
    ...
    if site == "news":
        search_url = f"https://inidblogs.com/?s={search_query}"
        html_text = requests.get(search_url).text
        soup = bs4.BeautifulSoup(html_text, "html.parser")
        search_result = soup.find_all("h2", {"class": "entry-title"})

        if search_result:
            result = f"<b>Search results for</b> <code>{html.escape(search_query)}</code> <b>on</b> IndiBlogs: \n"
            for entry in search_result:
                post_link = "https://inidblogs.com/" + entry.a["href"]
                post_name = html.escape(entry.text)
                result += f"• <a href='{post_link}'>{post_name}</a>\n"
        else:
            more_results = True
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> IndiBlogs"
        ...
        for entry in search_result:
            ...
            if entry.text.strip() == "Nothing Found":
                result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>IndiAnimeo</code>"
                more_results = False
                break
